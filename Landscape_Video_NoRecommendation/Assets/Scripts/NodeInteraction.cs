...
public class NodeInteraction : MonoBehaviour
{
    ...
    private void HandleObjectSelected(GameObject selectedObject)
    {
        ...
        if (selectedObject != null)
        {
            ...
            {
                // 如果有上次点击的节点，保持它的高亮状态
                if (lastClickedNode != null && lastClickedNode != selectedObject)
                {
                    // 如果是新的节点，则高亮当前节点并保留之前的高亮
                    HighlightManager.Instance.HighlightNode(selectedObject);
                }
            ...
            // 更新最后点击的节点
            lastClickedNode = selectedObject;
        }
    }
    // 在应用程序退出时调用
